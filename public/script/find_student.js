...
$(document).ready(function () {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        $.each(response.response.list, function (index, item) {
          ...
          var name = $("<td>").append($("<a>").attr("href", "/student/history/" + encodeURIComponent(item.first_name.replace(/\s+/g, '-') + "-" + item.last_name.replace(/\s+/g, '-')) + "/" + item.student_id).text(item.first_name + " " + item.last_name));
          ...
          button.find("button").click(function () {
            ...
            // Pass any necessary data using variables in the current scope
            if (confirm("ARE SURE DEACTIVATE")) {
              apiDeactivate(item.student_id);
            }
          });
          row.append(
            number,
  ...
  const apiDeactivate = function (student_id) {
    $.ajax({
      url: "/api/find/studentDeactivate",
      type: "POST",
      dataType: "json",
